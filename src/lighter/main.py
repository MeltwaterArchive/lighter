...
def compare_service_versions(nextVersion, prevVersion, path=''):
    if isinstance(nextVersion, dict):
        ...
        if isinstance(
                ...
                int) and isinstance(
                prevVersion,
                int) and nextVersion == 0 and path == '/ports':
            return True
        logging.debug(
            "Value has changed at %s (%s != %s)",
            path,
            nextVersion,
            prevVersion)
        return False
...
def parse_service(filename, targetdir=None, verifySecrets=False):
    ...
    if 'env' in config:
        for key, value in config['env'].iteritems():
            ...
            )) and 'public' not in key.lower()) and len(secretary.extractEnvelopes(value)) == 0:
                if verifySecrets:
                    raise RuntimeError(
                        'Found unencrypted secret in %s: %s' %
                        (filename, key))
                else:
                    logging.warn(
                        'Found unencrypted secret in %s: %s' %
    ...
    if targetdir:
        ...
        # Exception if directory exists, e.g. because another thread created it
        # concurrently
        try:
...
def parse_services(filenames, targetdir=None, verifySecrets=False):
    ...
    return Parallel(
        ...
        delayed(parse_service)(filename, targetdir, verifySecrets)
        for filename in filenames)


def get_marathon_url(url, id, force=False):
    return url.rstrip(
        '/') + '/v2/apps/' + id.strip('/') + (force and '?force=true' or '')
    ...
    for service in services:
        try:
            ...
            if not targetMarathonUrl:
                raise RuntimeError(
                    "No Marathon URL defined for service %s" %
                    service.filename)

            parsedMarathonUrl = urlparse(targetMarathonUrl)
            appurl = get_marathon_url(
                targetMarathonUrl, service.config['id'], force)
            ...
            if compare_service_versions(service.config, prevVersion):
                logging.info(
                    "Service already deployed with same config: %s",
                    service.filename)
                continue
            ...
            datadog.notify(
                id=service.id,
                title="Deployed %s to the %s environment" %
                (service.id,
                 service.environment),
            ...
            raise RuntimeError(
                ...
                (service.filename, e.code, e, e.read())), None, sys.exc_info()[2]
        except urllib2.URLError as e:
            raise RuntimeError(
                "Failed to deploy %s (%s)" %
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-n', '--noop', dest='noop',
        help='Execute dry-run without modifying Marathon [default: %(default)s]',
    ...
    parser.add_argument(
        ...
        '--targetdir',
        dest='targetdir',
        help='Directory to output rendered config files',
        default=None)
    ...
    verify_parser = subparsers.add_parser(
        'verify', prog='lighter', usage='%(prog)s verify YMLFILE...',
    ...
    verify_parser.add_argument(
        '--verify-secrets', dest='verifySecrets',
        help='Fail verification if unencrypted secrets are found [default: %(default)s]',
        action='store_true', default=False)
    ...
    try:
        if args.command == 'deploy':
            ...
            verify(
                args.filenames,
                targetdir=args.targetdir,
                verifySecrets=args.verifySecrets)
