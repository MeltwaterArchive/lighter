...
class SecretaryTest(unittest.TestCase):
    ...
    def testAddMasterKey(self):
        ...
        self.assertEquals(
            service.config['env']['MASTER_PUBLIC_KEY'],
            'pq01FdTbzF7q29HiX8f01oDfQyHgVFw03vEZes7OtnQ=')
    ...
    def testRedeployWithoutChange(self):
        service1 = lighter.parse_service(
            'src/resources/yaml/staging/myservice-servicekey.yml')
        service2 = lighter.parse_service(
            'src/resources/yaml/staging/myservice-servicekey.yml')
        self.assertNotEqual(service1.config, service2.config)

        self.assertTrue(
            lighter.compare_service_versions(
                service1.config,
                service2.config))
        self.assertTrue(
            lighter.compare_service_versions(
                service1.config, json.loads(
                    util.toJson(
                        service2.config))))

        self.assertNotEqual(
            service1.config['env']['DEPLOY_PUBLIC_KEY'],
            service2.config['env']['DEPLOY_PUBLIC_KEY'])
        self.assertNotEqual(
            service1.config['env']['DEPLOY_PRIVATE_KEY'],
            service2.config['env']['DEPLOY_PRIVATE_KEY'])

    def testServiceWithoutSecrets(self):
        service = lighter.parse_service(
            'src/resources/yaml/staging/myservice-nosecret.yml')
    ...
    def testExtractEnvelopes(self):
        ...
        self.assertEqual(
            ["ENC[NACL,uSr123+/=]", "ENC[NACL,pWd123+/=]"],
            envelopes)
