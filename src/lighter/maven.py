...
class VersionRange(object):
    ...
    def parseVersion(version):
        if version:
            return tuple(
                int(digit)
                for digit in VersionRange.SPLIT.split(version.split('-')[0])
                if digit.isdigit())
        return None
...
class Artifact(object):
    ...
    def __init__(self, version, uniqueVersion, classifier, body):
        ...
        self.uniqueVersion = (
            uniqueVersion or version) + (classifier and ('-' + classifier) or '')
        self.body = body
...
class ArtifactResolver(object):
    ...
    def fetch(self, version):
        ...
        # Try to resolve unique/timestamped snapshot versions from
        # maven-metadata.xml
        logging.debug(
            'Trying to resolve %s to a unique timestamp-buildnumber version',
            version)
        url = '{0}/{1}/{2}/{3}/maven-metadata.xml'.format(
            self._url, self._groupid.replace(
                '.', '/'), self._artifactid, version)
        metadata = {}
        ...
        # Find a matching snapshot version (Gradle doesn't create
        # <snapshotVersions> but Maven does)
        timestamp = util.rget(metadata, 'versioning', 'snapshot', 'timestamp')
        buildNumber = util.rget(
            metadata,
            'versioning',
            'snapshot',
            'buildNumber')
        snapshot = '-'.join(
            filter(
                bool,
                [version[0: len(version) - len(trailer)],
                 timestamp, buildNumber])) if(
            timestamp is not None and buildNumber is not None) else None
        return self._fetch(version, snapshot, metadata)

    def _fetch(self, version, uniqueVersion=None, metadata={}):
        url = '{0}/{1}/{2}/{3}/{2}-{4}'.format(
            self._url, self._groupid.replace('.', '/'),
            self._artifactid, version, uniqueVersion or version)
        ...
        if not uniqueVersion:
            ...
            if timestamp or buildNumber:
                uniqueVersion = '-'.join(filter(bool,
        ...
        try:
            return Artifact(
                ...
                uniqueVersion,
                self._classifier,
                util.jsonRequest(url))
        except urllib2.HTTPError as e:
            raise RuntimeError(
                "Failed to retrieve %s HTTP %d (%s)" %
                (url, e.code, e)), None, sys.exc_info()[2]
    ...
    def resolve(self, expression):
        metadata = util.xmlRequest(
            '{0}/{1}/{2}/maven-metadata.xml'.format(
                self._url, self._groupid.replace(
                    '.', '/'), self._artifactid))
        versions = util.toList(
            util.rget(
                metadata,
                'versioning',
                'versions',
    ...
    def selectVersion(self, expression, versions):
        ...
        if not matches:
            raise RuntimeError(
                'Failed to find a version that matches %s' %
                expression)
        return matches[-1]
